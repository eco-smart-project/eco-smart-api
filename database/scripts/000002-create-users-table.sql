create sequence ecosmart.seq_users;

create table ecosmart.users
(
  id         bigint primary key default nextval('ecosmart.seq_users'),
...
comment on column ecosmart.users.type is 'A: Athlete, N: Nutritionist, P: Personal';
comment on column ecosmart.users.status is 'A: Active, I: Inactive, D: Deleted';
...
create trigger trg_users_biu_er
  before insert or update on ecosmart.users
  for each row execute function fnc_trg_users_biu_er();

insert into ecosmart.users (id, type, username, name, email, password, status, created_at, updated_at, deleted_at)
